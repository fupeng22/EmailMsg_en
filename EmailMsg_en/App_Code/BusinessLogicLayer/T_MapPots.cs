...
public class T_MapPots
{
    ...
    /// <param name="MapId"></param>
    /// <returns></returns>
    public DataSet GetAlrmPotInfoByMapId(int MapId)
    {
        DataSet ds = null;
        string strSQL = "";
        strSQL = string.Format(@"SELECT  TMP.[Id] ,
                                        TMP.[MapId] ,
                                        TMP.[EquipmentId] ,
                                        TMP.[MapPotName] ,
                                        TMP.[posX] ,
                                        ...
                                        TMP.[Width] ,
                                        TMP.[Height] ,
                                        VDRM.ID AS ReceiveMsgID ,
                                        VDRM.ReadFlag ,
                                        VDRM.ReceiveMsg ,
                                        VDRM.ReceiveTime
                                FROM    V_Distinct_ReceiveMsg VDRM
                                        INNER JOIN dbo.T_MapPots TMP ON TMP.EquipmentId = VDRM.EquipmentId
                                WHERE   TMP.MapId = {0}
                                        AND VDRM.ReadFlag = 0", MapId);
        ds = SqlServerHelper.Query(strSQL);
